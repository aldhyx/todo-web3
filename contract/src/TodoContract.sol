...
contract TodoContract {
    ...
    function createTodo(string calldata _content) external onlyRegistered returns (Todo memory) {
        if (bytes(_content).length == 0) {
            revert Todo_Empty_Content_Not_Allowed();
        }

        uint256 newId = userTodoIds[msg.sender].length + 1;
        Todo memory newTodo = Todo({content: _content, createAt: block.timestamp, isComplete: false, id: newId});
    ...
    function checkTodoById(uint256 _todoId) external onlyRegistered OnlyExistTodo(_todoId) {
        todoList[msg.sender][_todoId].isComplete = true;
        emit TodoChecked(msg.sender, _todoId);
    }

    function uncheckTodoById(uint256 _todoId) external onlyRegistered OnlyExistTodo(_todoId) {
        todoList[msg.sender][_todoId].isComplete = false;
        emit TodoUnchecked(msg.sender, _todoId);
    }

    function getTodoById(uint256 _todoId)
        external
        view
        onlyRegistered
